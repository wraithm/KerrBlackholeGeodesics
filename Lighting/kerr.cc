...
void drawKerr()
{
    ...
    if(iterations <= HISTORY)
    {
        ...
        //historyx[iterations] = (float) sqrt(pow(Y[2], 2) + pow(ANGM/MASS, 2)) * cos(Y[4]) * sin(Y[3]);
        //historyy[iterations] = (float) Y[2] * cos(Y[3]);
        //historyz[iterations] = (float) sqrt(pow(Y[2], 2) + pow(ANGM/MASS, 2)) * sin(Y[3]) * sin(Y[4]);
...
void drawTrace()
{
    ...
    /*
    glBegin(GL_LINE_STRIP);
        glVertex3f(historyx[iterations], historyy[iterations], historyz[iterations]);
        glVertex3f(historyx[iterations - 1], historyy[iterations - 1], historyz[iterations - 1]);
        glVertex3f(0.0f, 0.0f, 0.0f);
    glEnd();
    */
}
